...
enum ConfigError parse_list(
    ...
    *count = 0;
    ...
    snprintf(param_pattern, sizeof(param_pattern), "\\s*%s\\s*:", param_name);
    ...
    regex_t line_regex;
    //compile_regex(&line_regex, "\\n(\\s+)([-*>]?\\s*)([^\\n]*)") < 0
    if (compile_regex(&line_regex, "(\\s+)([-*>]?\\s*)([^\\n]*)") < 0) {
    ...
    regex_t base_regex;
    //compile_regex(&base_regex, "\\n(\\s+)") < 0
    if (compile_regex(&base_regex, "(\\s+)") < 0) {
