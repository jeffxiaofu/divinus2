...
int i6_config_load(char *path, int ExpoTimeMs)
{
    ...
    if (ExpoTimeMs)
    {
        ...
        //_i6_isp_expo.u32ISPGain = 1;
        float SensorGain = 4.5;
        _i6_isp_expo.u32SensorGain = SensorGain * 1024;
        if (ret = i6_isp.fnSetManualExpo(_i6_isp_chn, &_i6_isp_expo))
            return ret;
        HAL_INFO("i6_hal", "FN:%d ExpoTimeMs:%d SensorGain:%.2f\n", _i6_isp_expo.u32FNx10 / 10, ExpoTimeMs, SensorGain);
    }
    return ret;
}
...
int i6_pipeline_create(char sensor, short width, short height, char framerate)
{
...

    {
        ...
        port.field = 0;
        port.interlaceOn = 0;
        port.pixFmt = (i6_common_pixfmt)(_i6_snr_plane.bayer > I6_BAYER_END ? _i6_snr_plane.pixFmt : (I6_PIXFMT_RGB_BAYER + _i6_snr_plane.precision * I6_BAYER_END + _i6_snr_plane.bayer));
    ...
    if (series == 0xF1)
    {
        ...
        channel.pixFmt = (i6_common_pixfmt)(_i6_snr_plane.bayer > I6_BAYER_END ? _i6_snr_plane.pixFmt : (I6_PIXFMT_RGB_BAYER + _i6_snr_plane.precision * I6_BAYER_END + _i6_snr_plane.bayer));
...
int i6_video_create(char index, hal_vidconfig *config)
{
    ...
    else if (config->codec == HAL_VIDCODEC_H265)
    {
        ...
        {
        case HAL_VIDMODE_CBR:
            channel.rate.mode = series == 0xEF ? I6OG_VENC_RATEMODE_H265CBR : I6_VENC_RATEMODE_H265CBR;
            channel.rate.h265Cbr = (i6_venc_rate_h26xcbr){.gop = config->gop,
                                                          .statTime = 1,
                                                          .fpsNum = config->framerate,
                                                          .fpsDen = 1,
                                                          .bitrate =
                                                              (unsigned int)(config->bitrate) << 10,
                                                          .avgLvl = 1};
            break;
        case HAL_VIDMODE_VBR:
            channel.rate.mode = series == 0xEF ? I6OG_VENC_RATEMODE_H265VBR : I6_VENC_RATEMODE_H265VBR;
            ...
            break;
        case HAL_VIDMODE_QP:
            channel.rate.mode = series == 0xEF ? I6OG_VENC_RATEMODE_H265QP : I6_VENC_RATEMODE_H265QP;
            channel.rate.h265Qp = (i6_venc_rate_h26xqp){.gop = config->gop,
                                                        .fpsNum = config->framerate,
                                                        .fpsDen = 1,
                                                        .interQual = config->maxQual,
                                                        .predQual = config->minQual};
            break;
        case HAL_VIDMODE_ABR:
            HAL_ERROR("i6_venc", "H.265 encoder does not support ABR mode!");
        case HAL_VIDMODE_AVBR:
            channel.rate.mode = series == 0xEF ? I6OG_VENC_RATEMODE_H265AVBR : I6_VENC_RATEMODE_H265AVBR;
    ...
    else if (config->codec == HAL_VIDCODEC_H264)
    {
        ...
        {
        case HAL_VIDMODE_CBR:
            channel.rate.mode = series == 0xEF ? I6OG_VENC_RATEMODE_H264CBR : I6_VENC_RATEMODE_H264CBR;
            ...
            break;
        case HAL_VIDMODE_VBR:
            channel.rate.mode = series == 0xEF ? I6OG_VENC_RATEMODE_H264VBR : I6_VENC_RATEMODE_H264VBR;
            ...
            break;
        case HAL_VIDMODE_QP:
            channel.rate.mode = series == 0xEF ? I6OG_VENC_RATEMODE_H264QP : I6_VENC_RATEMODE_H264QP;
        ...
        case HAL_VIDMODE_ABR:
            ...
            channel.rate.h264Abr = (i6_venc_rate_h26xabr){.gop = config->gop,
                                                          ...
                                                          .maxBitrate = (unsigned int)(config->maxBitrate) << 10};
            break;
        case HAL_VIDMODE_AVBR:
        ...
        HAL_ERROR("i6_venc", "This codec is not supported by the hardware!");
...
attach:
    ...
    HAL_INFO("i6_venc", "MinQp:%d,MaxQp:%d,IPQPDelta:%d\n",
    ...
    {
        // 帧内刷新
        i6_venc_IntraRefresh intraRefreshConfig = {.Enable = 1, .RefreshLineNum = 2, .ReqIQp = 1};
        if (ret = i6_venc.fnSetIntraRefresh(index, &intraRefreshConfig))
        {
            HAL_ERROR("i6_venc", "SetIntraRefresh error:%x\n", ret);
            return ret;
        }
        HAL_INFO("i6_venc", "IntraRefresh is enabled!\n");
    }
                            ...
                            if (series == 0xEF)
                            {
                                ...
                                case HAL_VIDCODEC_H264:
                                    for (unsigned int p = 0; p < pack->length - 4; p++)
                                    {
                                        ...
                                        if (outPack[j].data[p] == 0 && outPack[j].data[p + 1] == 0 &&
                                            outPack[j].data[p + 2] == 0 && outPack[j].data[p + 3] == 1)
                                        {
                                            nalu_start_offset = 4;
                                        }
                                        else if (outPack[j].data[p] == 0 && outPack[j].data[p + 1] == 0 &&
                                                 outPack[j].data[p + 2] == 1)
                                        {
                                            nalu_start_offset = 3;
                                        }
                                        else
                                        ...
                                        outPack[j].nalu[n].type = outPack[j].data[p + nalu_start_offset] & 0x1F;
                                        outPack[j].nalu[n++].offset = p;
                                        if (n == (outPack[j].naluCnt))
                                            break;
                                    }
                                    break;
                                case HAL_VIDCODEC_H265:
                                    ...
                                    {
                                        // if (outPack[j].data[p] || outPack[j].data[p + 1] ||
                                        //     outPack[j].data[p + 2] || outPack[j].data[p + 3] != 1)
                                        //     continue;
                                        int nalu_start_offset = 0;
                                        if (outPack[j].data[p] == 0 && outPack[j].data[p + 1] == 0 &&
                                            outPack[j].data[p + 2] == 0 && outPack[j].data[p + 3] == 1)
                                        ...
                                        outPack[j].nalu[n].type = (outPack[j].data[p + nalu_start_offset] & 0x7E) >> 1;
                                        outPack[j].nalu[n++].offset = p;
                                        if (n == (outPack[j].naluCnt))
                                            break;
                                    }
                                ...
                                outPack[j].naluCnt = n;
                                outPack[j].nalu[n].offset = pack->length;
                                for (n = 0; n < outPack[j].naluCnt; n++)
                                    outPack[j].nalu[n].length =
